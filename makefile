...
$(SCENES_PATTERN) &: $(DIR_IN)/% $(QUALIFIERS_OUT)
	$(eval $(call configure, $<))
	$(V_ENC) -y -framerate $(FPS) -pattern_type glob -i "$</*$(EXT_IN)" $(foreach HEIGHT, $(SIZES), -vf scale=-1:$(HEIGHT) $(V_ENC_FLAGS) $(DIR_OUT)/$(HEIGHT)/$*$(EXT_OUT))
